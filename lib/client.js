window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function filterGroups(groups, hidden, disabledProviders, current) {
      ...
      return (groups || []).flatMap((group) => {
        const models = (group.models || []).filter((model) => {
          ...
          return selected || (!disabled.has(group.id) && !hiddenModels.has(keyOf(group.id, model.id)));
        });
    ...
    function installModelSelectionFilter(ctx, connection, remote) {
      ...
      const wrapDirectory = (directory) => {
        if (!directory || wrapped.has(directory) || typeof directory.load !== 'function' || !directory.store?.update) return;
        wrapped.add(directory);
        const load = directory.load.bind(directory);
        directory.load = async () => {
          await refreshControl();
          const result = await load();
          directory.store.update((state) => {
            state.groups = filterGroups(state.groups, control.hidden, control.disabledProviders, state.current);
          });
          return {
            ...result,
            groups: filterGroups(result.groups, control.hidden, control.disabledProviders, result.current),
          };
        };
      };
      ...
      remote?.$on?.('settings/document-updated', (namespace) => {
        if (namespace === CONTROL_NS) refreshDirectories();
      });
      refreshDirectories();
    }
    ...
    function apply(ctx) {
      ...
      installModelSelectionFilter(ctx, connection, remote);
