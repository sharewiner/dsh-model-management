...
export function createModelDirectoryCompat({ directories, getControl, onIncompatible = () => {} }) {
  ...
  const wrappedDirectoryFor = function directoryForWithVisibility(sessionId) {
  ...
  const handle = {
    ...
    dispose() {
      ...
      for (const [directory, methods] of wrapped) {
        if (directory.load === methods.wrappedLoad) directory.load = methods.originalLoad;
      }
      wrapped.clear();
